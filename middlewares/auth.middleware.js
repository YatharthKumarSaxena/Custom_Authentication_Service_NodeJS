...
const verifySignUpBody = async (req,res,next) =>{
    ...
    try{
        ...
        // Check if Email ID Field Provided Is It Valid Or Not
        if(typeof emailID === 'string' && emailID.length){
            if (!validateLength(emailID,emailLength.min,emailLength.max)){
                logMiddlewareError("Sign Up, Invalid Email ID Length",req);
                return throwInvalidResourceError(res,`Email ID , Email ID must be of minimum ${emailLength.min} letters and maximum ${emailLength.max} letters`);
            }
            if(!emailRegex.test(emailID)){
                logMiddlewareError("Sign Up, Invalid Email format",req);
                return throwInvalidResourceError(res,"Email ID must be a valid email address.");
            }
        }
